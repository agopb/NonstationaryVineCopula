...
normtvtp=function(theta,data,rhobar,Call){
  ...
  for (jj in  2:T){
    if (jj<=10){
      psi1[jj] = w+ a*kappa[jj-1] + b*(mean((x[1:jj-1]*y[1:jj-1])))
    }else{
      psi1[jj] = w+ (a*kappa[jj-1]) + b*(mean((x[(jj-10):(jj-1)]*y[(jj-10):(jj-1)])))
    }
...
dynamic_normal=function(data,theta,rhobar1, plot){
  lower =rep(-5,3)
  upper =rep(5,3)
  
  model <- optim(theta,normtvtp,data=data,rhobar=rhobar1, Call="optim",
  ...
  rhot=normtvtp(coef,data=data,rhobar=rhobar1,Call="filtering")$TVTP
  if ( plot==TRUE){
    plot(ts( rhot[5:n]))}
