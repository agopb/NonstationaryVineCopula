...
clayton_tvtp=function(theta,data,rhobar,Call){
  ...
  for (jj in  2:T){
    ...
    kappa[jj] = psi1*psi1;		# a modified logistic transformation
  }
  ...
  if(sum(is.infinite(rhohat))){
    ...
    CL=rep(0,T)
    for (i in 1:T){
      CL[i]=clayton_pdf(u[i],v[i],rhohat[i])
    }
    CL = sum(log(CL))
  }
...
dynamic_clayton=function(data,theta,rhobar1, plot){
  lower =rep(-5,3)
  upper =rep(5,3)
  
  model <- optim(theta,clayton_tvtp,data=data,rhobar=rhobar1, Call="optim",
  ...
  rhot=clayton_tvtp(coef,data=data,rhobar=rhobar1,Call="filtering")$TVTP
  if ( plot==TRUE){
    plot(ts( rhot[5:n]))}
